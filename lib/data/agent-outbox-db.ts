 ...
 * per `(generated_post_id, property_id)` — repeated calls (e.g. user
 * re-publishes after a partial failure) update the same row instead of
 * stacking duplicates.
 *
 * 2026-07-23 (John) — idempotency widened from per-post to per-(post,
 * property) so a multi-property Open House post can notify EVERY listing
 * agent featured in it, not just the anchor property's agent (the 7/23
 * OH post notified 1 of 7). Single-listing posts behave exactly as
 * before: same post + same property resolves to the same single row.
 *
...
export async function createOutboxRowForPost(args: {
  ...
  // Idempotency: upsert by (generated_post_id, property_id). If a row
  // already exists for this post+property we refresh content snapshot +
  // URLs but PRESERVE sent_at / acknowledged_at so we don't lose the
  // delivery history. Legacy rows (pre-2026-07-23, one per post) still
  // match here because their property_id was the anchor property.
